...
namespace App\Filament\Resources\Visas\Widgets;
...
class VisaStatusWidget extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        return [
            Stat::make('لم يتم التقديم', Visa::where('status', VisaStatus::NOT_APPLIED)->count())
            ...
            Stat::make('تمت الموافقة', Visa::where('status', VisaStatus::APPROVED)->count())
                ->description($total > 0 ? round(Visa::where('status', VisaStatus::APPROVED)->count() / $total * 100).'%' : '0%')
                ->color('success')
                ->icon('heroicon-o-check-badge'),

            Stat::make('مرفوض', Visa::where('status', VisaStatus::REJECTED)->count())
