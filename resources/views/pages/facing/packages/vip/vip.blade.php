<div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8" dir="rtl">
    <div class="mb-8 text-center">
        <flux:heading size="xl">باقات VIP و VVIP</flux:heading>
        <p class="mt-2 text-zinc-600 dark:text-zinc-400">أفخم الباقات وأرقى الخدمات لضيوف الرحمن</p>
    </div>
    ...
    @if ($packages->isEmpty())
        <div class="rounded-xl border border-dashed border-zinc-300 py-16 text-center dark:border-zinc-600">
            ...
            <p class="mt-4 text-lg text-zinc-500 dark:text-zinc-400">لا توجد باقات VIP متاحة حالياً</p>
        </div>
    @else
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            @foreach ($packages as $package)
                <a href="{{ route('packages.show', $package) }}" wire:navigate
                    class="group block rounded-xl border border-amber-200 bg-white p-5 shadow-sm
                           transition hover:border-amber-400 hover:shadow-md
                           dark:border-amber-800 dark:bg-zinc-800 dark:hover:border-amber-500">
                    <div class="mb-3 flex items-center justify-between">
                        <h3 class="text-lg font-bold text-zinc-900 group-hover:text-amber-600 dark:text-white dark:group-hover:text-amber-400">
                            {{ $package->name }}
                        </h3>
                    </div>
                    ...
                    <div class="space-y-2 text-sm text-zinc-600 dark:text-zinc-400">
                        <div class="flex items-center gap-2">
                            <flux:icon.currency-dollar class="size-4 text-amber-600" />
                    ...
                    <div class="mt-4">
                        <span
                            class="inline-flex items-center gap-1 text-sm font-medium text-amber-600 transition group-hover:gap-2 dark:text-amber-400">
                            عرض التفاصيل
