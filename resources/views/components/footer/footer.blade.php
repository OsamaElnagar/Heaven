<footer
    ...
    <div class="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        <div class="sm:col-span-2 lg:col-span-1">
            <div class="flex items-center gap-3 mb-4">
                <div
                    ...
                    <x-app-logo-icon class="size-5 fill-current text-white dark:text-black" /></div>
                <div class="flex flex-col leading-tight">
                    <span class="text-lg font-bold text-stone-900 dark:text-stone-100">{{ config('app.name') }}</span>
            ...
            <div class="flex flex-wrap gap-2">
                <span
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded border border-stone-200
        ...
        <div>
            ...
            <ul class="space-y-2.5">
                @foreach ([['label' => 'من نحن', 'route' => 'about'], ['label' => 'تتبع حجزك', 'route' => 'track'], ['label' => 'الأسئلة الشائعة', 'route' => 'faq'], ['label' => 'المعرض', 'route' => 'gallery'], ['label' => 'تواصل معنا', 'route' => 'contact']] as $link)
                    <li>
                ...
                @foreach ([['icon' => 'heroicon-o-phone', 'label' => 'الهاتف', 'value' => '+20 10 0000 0000'], ['icon' => 'heroicon-o-envelope', 'label' => 'البريد الإلكتروني', 'value' => config('app.email')], ['icon' => 'heroicon-o-map-pin', 'label' => 'العنوان', 'value' => 'القاهرة، مصر الجديدة']] as $item)
                    <div class="flex items-start gap-3">
                        <div
                            ...
                            <x-dynamic-component :component="$item['icon']" class="w-3.5 h-3.5 text-[#C9A84C]" />
                        </div>
                        <div>
    ...
    <div class="border-t border-stone-200 dark:border-stone-800">
        <div class="max-w-7xl mx-auto px-6 py-4 flex flex-wrap items-center justify-between gap-3">
            <p class="text-xs text-stone-400 dark:text-stone-500">
                © {{ date('Y') }} <span class="text-[#C9A84C]">{{config('app.name')}} للحج والعمرة</span>. جميع الحقوق محفوظة.
            </p>
